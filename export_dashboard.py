...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .sort-row { display: flex; align-items: center; gap: 10px; margin: 8px 0; flex-wrap: wrap; }
  .sort-row label { font-size: 13px; color: #333; }
  .sort-row select { font-size: 13px; padding: 4px 8px; }
  .custom-order-panel { margin: 6px 0 10px; }
  ol.order-list { list-style: none; margin: 0; padding: 0; max-width: 420px; border: 1px solid #eee;
                  border-radius: 4px; background: #fff; }
  .order-item { display: flex; align-items: center; gap: 8px; padding: 4px 8px; border-bottom: 1px solid #f2f2f2;
                cursor: grab; }
  .order-item:last-child { border-bottom: none; }
  .order-item.dragging { opacity: 0.4; }
  .order-item span { flex: 1; font-size: 13px; }
  .order-btn { font-size: 11px; padding: 2px 6px; border: 1px solid #ccc; background: #f5f5f5; border-radius: 3px;
               cursor: pointer; }
  ...
  h2.sec-h { cursor: pointer; user-select: none; display: flex; align-items: center; gap: 6px; }
  h2.sec-h:hover { opacity: 0.8; }
  h2.sec-h .arrow { font-size: 13px; color: #378ADD; width: 14px; display: inline-block; text-align: center; }
  .sec-content.is-collapsed { display: none; }
  .collapse-controls { display: flex; gap: 6px; }
</style>
</head><body>
<header>
  <h1>台股資金流向儀表板</h1>
  ...
  <div class="collapse-controls">
    <button type="button" class="toggle" id="expandAllBtn">全部展開</button>
    <button type="button" class="toggle" id="collapseAllBtn">全部收合</button>
  </div>
...
<section id="overview">
  <h2 class="sec-h" data-section="overview" role="button" tabindex="0" aria-expanded="true"><span class="arrow" aria-hidden="true">▼</span>總覽</h2>
  <div class="sec-content" id="overview-content">
...
<section id="heatmap">
  <h2 class="sec-h" data-section="heatmap" role="button" tabindex="0" aria-expanded="true"><span class="arrow" aria-hidden="true">▼</span>板塊熱力圖</h2>
  <div class="sec-content" id="heatmap-content">
  <p class="note">34 個官方產業別板塊（依 3 年總金額活動量排序）x 150 週，色階：藍=淨流入、橘紅=淨流出、白=近零。
  滑鼠移到格子上可看板塊/週期間/金額。</p>
  <div class="sort-row">
    <label for="sectorSortMode">板塊排序：</label>
    <select id="sectorSortMode">
      <option value="activity">活動量（預設）</option>
      <option value="stroke">名稱筆劃</option>
      <option value="custom">自訂</option>
    </select>
    <button type="button" class="toggle" id="sectorOrderResetBtn">重設為預設</button>
  </div>
  <div class="custom-order-panel" id="sectorCustomOrderPanel" style="display:none;">
    <p class="note">拖曳排列，或用每列的 ↑/↓ 按鈕調整順序（無障礙備援）。排序結果會套用到下方熱力圖列順序與下鑽選單。</p>
    <ol class="order-list" id="sectorOrderList"></ol>
  </div>
...
<section id="ranking">
  <h2 class="sec-h" data-section="ranking" role="button" tabindex="0" aria-expanded="true"><span class="arrow" aria-hidden="true">▼</span>板塊排行與下鑽</h2>
  <div class="sec-content" id="ranking-content">
...
<section id="groups">
  <h2 class="sec-h" data-section="groups" role="button" tabindex="0" aria-expanded="true"><span class="arrow" aria-hidden="true">▼</span>族群視圖（概念股）</h2>
  <div class="sec-content" id="groups-content">
...
<section id="trust">
  <h2 class="sec-h" data-section="trust" role="button" tabindex="0" aria-expanded="true"><span class="arrow" aria-hidden="true">▼</span>投信特寫</h2>
  <div class="sec-content" id="trust-content">
...
<section id="notice">
  <h2 class="sec-h" data-section="notice" role="button" tabindex="0" aria-expanded="true"><span class="arrow" aria-hidden="true">▼</span>使用須知</h2>
  <div class="sec-content" id="notice-content">
...
const sectorDrillChartRef = makeDrill(
...
const groupDrillChartRef = makeDrill(
...
// ---- 板塊排序自訂（活動量/名稱筆劃/自訂，套用到熱力圖列順序 + 下鑽選單選項順序，
// 排行榜依資料排序、語意不同，不受影響） ----
const SECTOR_ORDER_STORAGE_KEY = 'twstockdb.sectorOrder.v1';
const activityOrder = DATA.sector_heat.labels.slice();
let currentSectorMode = 'activity';
let currentSectorOrder = activityOrder.slice();
let customSectorOrder = null; // 只在使用者切到「自訂」後才有值
let sectorDragIndex = null;

const sectorSortModeSelect = document.getElementById('sectorSortMode');
const sectorCustomPanel = document.getElementById('sectorCustomOrderPanel');
const sectorOrderListEl = document.getElementById('sectorOrderList');
const sectorOrderResetBtn = document.getElementById('sectorOrderResetBtn');
const sectorHeatTableEl = document.getElementById('sectorHeatTable');
const sectorDrillSelectEl = document.getElementById('sectorDrillSelect');

function loadSectorOrderPref() {
  try {
    const raw = localStorage.getItem(SECTOR_ORDER_STORAGE_KEY);
    if (!raw) return null;
    const parsed = JSON.parse(raw);
    if (!parsed || typeof parsed !== 'object') return null;
    return parsed;
  } catch (e) {
    // localStorage 不可用（例如隱私模式）：靜默降級，功能只在 session 內有效
    return null;
  }
}

function saveSectorOrderPref(mode, order) {
  try {
    localStorage.setItem(SECTOR_ORDER_STORAGE_KEY, JSON.stringify({ mode, order }));
  } catch (e) {
    // 同上，寫入失敗不影響功能，只是不會跨 session 記住
  }
}

function persistSectorOrder() {
  saveSectorOrderPref(currentSectorMode, customSectorOrder || []);
}

function sanitizeOrder(order, allLabels) {
  const allSet = new Set(allLabels);
  const filtered = (order || []).filter(name => allSet.has(name));
  const missing = allLabels.filter(name => !filtered.includes(name));
  return filtered.concat(missing);
}

function strokeSort(labels) {
  let collator;
  try {
    collator = new Intl.Collator('zh-Hant-TW-u-co-stroke');
  } catch (e) {
    collator = new Intl.Collator('zh-Hant');
  }
  return labels.slice().sort((a, b) => collator.compare(a, b));
}

function heatRowByLabel(heat, label) {
  const idx = heat.labels.indexOf(label);
  return idx >= 0 ? heat.matrix[idx] : heat.matrix[0].map(() => 0);
}

function reorderSelectOptions(selectEl, orderedLabels) {
  ...
  if (orderedLabels.includes(prevValue)) selectEl.value = prevValue;
}

function computeSectorOrder() {
  if (currentSectorMode === 'stroke') return strokeSort(activityOrder);
  if (currentSectorMode === 'custom') return sanitizeOrder(customSectorOrder, activityOrder);
  return activityOrder.slice();
}

function applySectorOrder() {
  currentSectorOrder = computeSectorOrder();
  const orderedHeat = {
    labels: currentSectorOrder,
    matrix: currentSectorOrder.map(label => heatRowByLabel(DATA.sector_heat, label)),
  };
  buildHeatTable(sectorHeatTableEl, orderedHeat, DATA.weeks);
  reorderSelectOptions(sectorDrillSelectEl, currentSectorOrder);
}

function renderSectorOrderList() {
  sectorOrderListEl.innerHTML = '';
  customSectorOrder.forEach((label, idx) => {
    const li = document.createElement('li');
    li.className = 'order-item';
    li.draggable = true;
    li.dataset.index = String(idx);
    li.addEventListener('dragstart', handleSectorDragStart);
    li.addEventListener('dragover', handleSectorDragOver);
    li.addEventListener('drop', handleSectorDrop);
    ...
    btnUp.type = 'button'; btnUp.className = 'order-btn'; btnUp.textContent = '↑';
    btnUp.setAttribute('aria-label', '上移 ' + label);
    btnUp.addEventListener('click', () => moveSectorItemUp(idx));
    li.appendChild(btnUp);

    const btnDown = document.createElement('button');
    btnDown.type = 'button'; btnDown.className = 'order-btn'; btnDown.textContent = '↓';
    btnDown.setAttribute('aria-label', '下移 ' + label);
    btnDown.addEventListener('click', () => moveSectorItemDown(idx));
    li.appendChild(btnDown);

    sectorOrderListEl.appendChild(li);
  });
}

function moveSectorItemUp(idx) {
  if (idx <= 0) return;
  const arr = customSectorOrder;
...
function moveSectorItemDown(idx) {
  if (idx >= customSectorOrder.length - 1) return;
  const arr = customSectorOrder;
...
function handleSectorDragStart(e) {
  sectorDragIndex = Number(e.currentTarget.dataset.index);
  e.currentTarget.classList.add('dragging');
  if (e.dataTransfer) e.dataTransfer.effectAllowed = 'move';
}

function handleSectorDragOver(e) {
  e.preventDefault();
  if (e.dataTransfer) e.dataTransfer.dropEffect = 'move';
}

function handleSectorDrop(e) {
  e.preventDefault();
  const targetIndex = Number(e.currentTarget.dataset.index);
  if (sectorDragIndex === null || sectorDragIndex === targetIndex) return;
  const item = customSectorOrder.splice(sectorDragIndex, 1)[0];
  customSectorOrder.splice(targetIndex, 0, item);
  sectorDragIndex = null;
  renderSectorOrderList();
  applySectorOrder();
  persistSectorOrder();
}

sectorSortModeSelect.addEventListener('change', () => {
  const newMode = sectorSortModeSelect.value;
  if (newMode === 'custom' && !customSectorOrder) {
    // 首次切到「自訂」：以當下顯示順序為初始值
    customSectorOrder = currentSectorOrder.slice();
  }
  currentSectorMode = newMode;
  sectorCustomPanel.style.display = newMode === 'custom' ? '' : 'none';
  if (newMode === 'custom') renderSectorOrderList();
  applySectorOrder();
  persistSectorOrder();
});

sectorOrderResetBtn.addEventListener('click', () => {
  customSectorOrder = null;
  currentSectorMode = 'activity';
  sectorSortModeSelect.value = 'activity';
  sectorCustomPanel.style.display = 'none';
  applySectorOrder();
  persistSectorOrder();
});

(function initSectorOrder() {
  const pref = loadSectorOrderPref();
  if (pref && pref.mode === 'custom' && Array.isArray(pref.order) && pref.order.length) {
    customSectorOrder = sanitizeOrder(pref.order, activityOrder);
    currentSectorMode = 'custom';
  } else if (pref && pref.mode === 'stroke') {
    currentSectorMode = 'stroke';
  } else {
    currentSectorMode = 'activity';
  }
  sectorSortModeSelect.value = currentSectorMode;
  sectorCustomPanel.style.display = currentSectorMode === 'custom' ? '' : 'none';
  if (currentSectorMode === 'custom') renderSectorOrderList();
  applySectorOrder();
})();
...
const trustChartRef = new Chart(document.getElementById('trustChart'), {
  type: 'bar',
  data: { labels: weekLabels, datasets: [{
...
// ---- 【第十四輪】區塊可收折：六大區塊（總覽/板塊熱力圖/板塊排行與下鑽/族群視圖/
// 投信特寫/使用須知）點標題整行切換收合/展開，狀態存 localStorage，重整/每日重產
// 頁面都保留使用者偏好；預設全部展開。Chart.js 陷阱：display:none 收合含 canvas 的
// 區塊後，重新展開時圖表尺寸會崩壞（寬度算成 0），所以展開時要對該區塊內的 Chart
// 實例呼叫 .resize()，見 chartsForSection()。 ----
const SECTION_COLLAPSE_STORAGE_KEY = 'twstockdb.sectionCollapse.v1';
const COLLAPSIBLE_SECTION_IDS = ['overview', 'heatmap', 'ranking', 'groups', 'trust', 'notice'];

function loadSectionCollapsePref() {
  try {
...
function saveSectionCollapsePref(state) {
  try {
...
const sectionCollapseState = loadSectionCollapsePref();

function chartsForSection(sectionId) {
  if (sectionId === 'overview') return [taiexChart, marketChart];
  if (sectionId === 'ranking') return [sectorDrillChartRef];
  if (sectionId === 'groups') return [groupDrillChartRef];
  if (sectionId === 'trust') return [trustChartRef];
  return [];
}

function resizeSectionCharts(sectionId) {
  chartsForSection(sectionId).forEach(c => {
    if (c && typeof c.resize === 'function') c.resize();
  });
}

function setSectionCollapsed(sectionId, collapsed, opts) {
  opts = opts || {};
  const header = document.querySelector('h2.sec-h[data-section="' + sectionId + '"]');
  ...
  if (!collapsed) resizeSectionCharts(sectionId);
  if (!opts.skipPersist) saveSectionCollapsePref(sectionCollapseState);
}

function toggleSection(sectionId) {
  setSectionCollapsed(sectionId, !sectionCollapseState[sectionId]);
}

function expandAllSections() {
  COLLAPSIBLE_SECTION_IDS.forEach(id => setSectionCollapsed(id, false, { skipPersist: true }));
  saveSectionCollapsePref(sectionCollapseState);
}

function collapseAllSections() {
  COLLAPSIBLE_SECTION_IDS.forEach(id => setSectionCollapsed(id, true, { skipPersist: true }));
  saveSectionCollapsePref(sectionCollapseState);
}

// 初始化：套用已存偏好（沒有存過就是空物件，等同全部展開）
COLLAPSIBLE_SECTION_IDS.forEach(id => {
  setSectionCollapsed(id, sectionCollapseState[id] === true, { skipPersist: true });
});

document.querySelectorAll('h2.sec-h').forEach(header => {
...
// 導覽列錨點：目標區塊若收合中，先展開再捲動過去
document.querySelectorAll('header nav a[href^="#"]').forEach(a => {
  a.addEventListener('click', (e) => {
    const targetId = a.getAttribute('href').slice(1);
    if (COLLAPSIBLE_SECTION_IDS.includes(targetId) && sectionCollapseState[targetId]) {
      e.preventDefault();
      setSectionCollapsed(targetId, false);
