...
def main():
    ...
    for weekday in range(5):
        ...
        options = dict(trailing=False, observation_days=None, equity_allocation=True, position_weight=.05, peak_stop=.30, signal_weekday=weekday, roll_holidays=True)
        ...
        (out / 'comparison.json').write_text(json.dumps(dict(fingerprint=fingerprint, excluded_dates=excluded, adjusted_high_rows=len(audit), certification='PRICE_PATH_DIAGNOSTIC_ONLY', settings=dict(initial_capital=1000000, position_weight=.05, peak_stop=.30, roll_holidays=True, execution='next_session_open', momentum_exit='month_end', observation_days=None, ma_exit=False), weekdays=summaries), ensure_ascii=False, indent=2), encoding='utf-8')
