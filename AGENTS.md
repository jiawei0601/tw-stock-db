...
- build / run：
  ...
  - `python refresh_daily.py [--dry-run]`（**【第十二輪】每日刷新腳本**，把上述十一個
    build/export 腳本依相依順序串成一條**嚴格串行**的更新鏈：`build_institutional_
    summary.py` → `build_daily_prices.py` → `build_taiex.py` → `build_revenue_
    history.py` → `build_fundamentals.py` → `build_sector_flow.py` → `build_sector_
    flow_weekly.py` → `build_sector_flow_value.py` → `build_group_flow.py` →
    `export_sector_flow_animation.py` → `export_dashboard.py`。每步用 `subprocess.run`
    執行，開始/結束/耗時/成功失敗記錄到 `data/refresh.log`（append；超過 5MB 自動砍掉
    前半只保留後半，避免無限增長）。**單步失敗不中止**——記錄後繼續跑後續步驟（增量
    抓取失敗隔天會自動補上，彙總/匯出步驟用現有資料照跑），全部跑完後若有任何失敗，
    透過 `C:\CLAUDE\tools\telegram\notify.py` 的 `send()` 發一則失敗摘要通知（比照
    tw-momentum-scanner 的 `notifier/telegram.py` 用法：sys.path 加入該目錄後
    `import notify`；**通知失敗一律吞掉，不可讓 refresh 當掉**）；全部成功時安靜結束
    不通知。`--dry-run` 只列印 11 步清單、不執行、不寫 log。exit code：全成功 0、有
    失敗 1（供排程系統判斷）。**本腳本本身不含排程註冊**，實際排程（建議週一至五
    18:30）由使用者另行授權後在主對話設定，見 HANDOFF.md 第十二輪紀錄。
...
export_dashboard.py                -> 【第十一輪，本專案主產出物】orchestrate：唯讀
                                  ...
                                  視圖+投信特寫+使用須知，不寫入資料庫）
refresh_daily.py                   -> 【第十二輪】orchestrate：依序 subprocess.run
                                  上述十一個 build/export 腳本（嚴格串行，SQLite
                                  單寫入者），開始/結束/耗時/成功失敗記錄
                                  data/refresh.log（超過 5MB 自動輪替）；單步失敗
                                  不中止、繼續跑後續步驟，結尾若有失敗才透過
                                  C:\CLAUDE\tools\telegram\notify.py 發摘要通知
                                  （通知失敗不擋 refresh）；--dry-run 只列印步驟；
                                  exit code 反映整體成敗；不寫入資料庫本身
```
