...
- build / run：
  ...
  - `python export_sector_flow_animation.py [--db-path PATH] [--top-n 20] [--out PATH]`
    （**【第八輪】** 從 `sector_flow_weekly` 匯出一份獨立靜態 HTML 動畫
    `analysis/sector-flow-weekly-animation.html`，只取活動量最大的 N 個板塊（預設 20，
    避免長條圖塞不下全部板塊看不清楚），瀏覽器直接開啟即可播放，不需要伺服器。
  - 七個 build/export 腳本彼此獨立、互不覆寫對方的表，可任意順序重跑（但不可同時併發跑，
    見上），`build_revenue_history.py`／`build_fundamentals.py`／
    `build_institutional_summary.py`／`build_sector_flow.py` 都依賴 `stocks` 已有資料，
    須先跑過 `build_db.py`；`build_sector_flow.py` 另外依賴 `institutional_flow_daily`
    已有資料；`build_sector_flow_weekly.py` 依賴 `build_sector_flow.py` 已跑過；
    `export_sector_flow_animation.py` 依賴 `build_sector_flow_weekly.py` 已跑過。
...
export_sector_flow_animation.py    -> 【第八輪】orchestrate：sector_flow_weekly 取活動量
                                  最大的 top-N 板塊 -> 匯出獨立靜態 HTML 動畫檔
```
...
3. **schema**：`stocks.stock_id` 為主鍵；`stock_groups` PK 是 `(stock_id, group_name)`。
   ...
   **【第八輪】** `sector_flow_weekly` PK 為 `(industry_name, week_index)`，是
   ...
   日曆週），範圍同樣是全市場。`export_sector_flow_animation.py` 不寫入資料庫，只讀取
   `sector_flow_weekly` 匯出獨立 HTML 檔，不屬於本節 schema 範圍。
