...
- **同日新增含興櫃研究（使用者明示）**：`compare_emerging_momentum.py`為獨立A/B/C比較入口；興櫃FinMind `open`是前日均價，絕不可拿來當當日開盤成交或當日高點。興櫃使用下一交易日VWAP代理、滑價敏感度及1%日成交股數容量；所得不得倒流支付同日上午買單。歷史登錄／終止資格需官方證據。此研究授權不自動修改Hetzner／券商實盤股票池。
...
- build / run：
  ...
  - `python export_dashboard.py [--db-path PATH] [--out PATH]`（**【第十一輪，本專案
    ...
    `build_revenue_history.py`／`build_fundamentals.py` 都已跑過。**【第十三輪】板塊
    熱力圖區塊新增排序自訂控制列**：活動量（預設）／名稱筆劃（`Intl.Collator`
    stroke collation）／自訂三種模式，自訂模式用 HTML5 拖放 + ↑/↓ 按鈕（無障礙備援）
    重排，排序結果套用到熱力圖列順序＋下鑽 `<select>` 選項順序（排行榜依資料排序，
    語意不同，不受影響）；純前端 `localStorage`（key `twstockdb.sectorOrder.v1`）
    持久化，跟資料庫/repo 無關，是**瀏覽器端**各自保存的個人化設定，載入時會過濾掉
    不存在的板塊名稱、把缺少的板塊補到清單末尾（防未來板塊清單變動），`localStorage`
    不可用（例如隱私模式）時 try/catch 靜默降級為 session 內有效，不影響其他功能。
    Pages 站台與本機 `file://` 開啟是不同 origin，`localStorage` 各自獨立不互通；
    每日排程重新匯出 `dashboard.html` 只覆寫檔案內容，不會清掉使用者瀏覽器裡已存的
    自訂排序。
