...
> 本檔為 HANDOFF.md 歷史輪次原文歸檔，最新現況看 HANDOFF.md。

## 第一輪～第十五輪原文（含當時的進行中／下一步／關鍵決策／雷區／怎麼跑）
...
## 2026-09-07 當日原始逐條記錄（已於 HANDOFF.md 合併整理為「現況與待辦」，此處保留原文未改寫）
...
- 2026-09-07 動能層回測（agy 實作、Claude 審查，一項揭露補正後 approve）：估值加動能仍未過 50%（M1 對 universe 38%、對 sub 52%）；純動能 C1 勝率 67%、超額中位 +2.4%；動能股裡再挑便宜的（C2）勝率掉到 30%。結論：這個市場先看動能、估值只用來排雷。附帶修掉訊號日基準價可能取到未來 5 日的前視漏洞，L0–L2 數字微調已在 summary 頂部揭露。

- 2026-09-07 動能為主（D0–D5）與轉強 T1 兩張工單完成（agy 實作、Claude 審查各一輪 request_changes 後修正 approve）：D5（動能前 1/4 排雷且營收為正）6 個月按月勝率 81%、超額中位 +5.5%；D3 組合層 CAGR 29.7%、MDD 26.9%、Sharpe 1.02（基準 25.5%／31.5%／0.87）。T1 轉強訊號領先 C1 中位 2 個月、6 個月內 78% 轉為 C1，但 T1 本身勝率僅 51.5%；族群廣度 G1 勝率 41%，不能當輪動偵測器。教訓：Codex 額度別花在看守迴圈（見 memory）。

- 2026-09-07 進場價格測試（agy 實作、Claude 審查 approve）：D5 訊號日收盤買（E0）全面優於等拉回／分批／突破確認；等拉回省 1.4–1.7% 但踏空 15% 部位，組合報酬少 3 個百分點；突破確認貴 6% 且踏空 25–30%。另測出場規則（backtest/stop_rule_test.py）：月線連兩天出場多頭月少賺 13.6pp，沒有簡單價格出場規則能保留收益。

- 2026-09-07 Codex 獨立審查（gpt-6-astra, reasoning high；報告 `docs/tasks/reports/codex-design-review.md`）：**總判定「需修正後可信」**。前三個最該先修：(1)【嚴重】比較母體與期間未統一——統一為相同 37 個月後 D0/D2/D3 勝率分別為 75.7%／78.4%／75.7%，原報告「品質排雷提升勝率」的論據不成立；(2)【嚴重】報酬與成交假設須先修（同收盤價進場、持有期間分割未還原、再平衡成本、把 2026-09-04 當完整月末）才談得上可交易超額；(3)【嚴重】凍結策略搜尋，補重疊樣本（六個月報酬高度重疊）的統計推論與真正未見資料，否則不足以認證穩健預測力。另註：審查時全套測試 280 通過、4 失敗，詳情見報告。
