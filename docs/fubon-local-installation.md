...
## 最新實測：登入成功（2026-09-08）

本機狀態檔確認 2026-09-08 23:18:42（台北時間）憑證 Python 解密成功、有效期檢查通過、券商登入成功，回傳 2 筆帳戶資訊，且登出完成。實際有效期限為 2027-01-28 23:59:59（台北時間）。這取代下方歷史紀錄中的待驗證狀態。

尚未同步成交／庫存、未下單、未設定排程自動登入。驗證成功不代表已完成 Hermes 券商串接。執行後 repo 出現未追蹤 `log/`；未檢視內容、未加入 git，後續串接應查核 SDK 日誌是否含敏感資料。
