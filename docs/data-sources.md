...
- `export_sector_flow_animation.py` 匯出的 HTML 是完全獨立檔案（Chart.js 走 CDN，
  ...
  跑 Python 的人）。
- **陷阱：瀏覽器強制深色模式會反轉整頁配色，但 Chart.js 座標軸刻度顏色是寫死的
  淺色系**，反轉後跟深色背景幾乎同色、板塊名稱幾乎看不到。實測發現後修正：
  `<html>` 加 `color-scheme: light`、`<body>` 明確指定白底黑字，讓頁面配色不受
  瀏覽器/系統深色模式影響（純靜態 HTML 沒有這個專案其他頁面可用的 CDS 深色模式
  自動適配變數，只能用這種方式固定配色）。

## 19. 【第八輪後續】TWSE 大盤加權指數（TAIEX）歷史日收盤 FMTQIK（2026-07-16 實測）

- URL：`https://www.twse.com.tw/rwd/zh/afterTrading/FMTQIK?date=YYYYMMDD&response=json`
- 需要瀏覽器 UA。`date` 參數只用來決定「查哪個月」，回傳**整個月份**的每日資料
  （例如 `date=20240601` 回傳 2024 年 6 月整月，第一筆是 06-03 而非 06-01，因為
  06-01/02 是週末非交易日），比照 `collectors/revenue_history.py` 用過的「查一個
  月、拿整月」模式，**不需要逐日查詢**——3 年歷史只要約 38 次請求（月份數）就能
  涵蓋，比三大法人的 750 次逐日請求便宜非常多。
- 欄位（實測 2024-06 樣本）：`日期`（民國年 yyy/mm/dd）、`成交股數`、`成交金額`、
  `成交筆數`、`發行量加權股價指數`（TAIEX 收盤，本專案只取這欄）、`漲跌點數`。
  數值欄位是千分位逗號字串，需轉型。
- 實測 2023-06 ~ 2026-07 共 38 個月、758 筆，零缺口、零請求失敗。
- **指數量級變化很大，測試合理值範圍不能抓太緊**：實測期間 TAIEX 從約 17,200
  （2023-06）漲到約 47,700（2026-06 尖峰），第一次寫 `tests/test_taiex.py` 時
  上限抓 40,000 直接測試失敗；查證後確認 44,000~47,700 這個高檔區間橫跨 2026-06
